...
@interface LCUserFeedbackViewController () <UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate, LCUserFeedbackReplyCellDelegate> {
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        _navigationBarStyle = LCUserFeedbackNavigationBarStyleNone;
        _contactHeaderHidden = NO;
//        _feedbackCellFont = [UIFont systemFontOfSize:14];
        _presented = YES;
    }
    return self;
}
...
- (UITableView *)tableView {
    if (_tableView == nil) {
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(self.navigationController.navigationBar.bounds), CGRectGetWidth(self.view.frame), CGRectGetHeight(self.view.frame) - kInputViewHeight - CGRectGetHeight(self.navigationController.navigationBar.bounds))
...
- (void)didFinishPickWithImage:(UIImage *)originImage
{
    [self prepareFeedbackWithBlock:^(BOOL succeeded, NSError *error) {
        if ([self filterError:error]) {
            AVFile *attachment = [AVFile fileWithName:@"feedback.png" data:UIImageJPEGRepresentation(originImage, 0.6)];
...
- (void)keyboardWillShow:(NSNotification *)notification {
    ...
    [UIView animateWithDuration:animationDuration
                     ...
                     animations:^{
                         ...
                         tableViewFrame.size.height = self.view.bounds.size.height - self.navigationController.navigationBar.frame.size.height - keyboardHeight - CGRectGetHeight(self.navigationController.navigationBar.bounds);
...
- (void)keyboardWillHide:(NSNotification *)notification {
    ...
    CGRect tableViewFrame = self.tableView.frame;
    tableViewFrame.origin.y = CGRectGetHeight(self.navigationController.navigationBar.bounds);
    tableViewFrame.size.height = CGRectGetHeight(self.view.frame) - kInputViewHeight - CGRectGetHeight(self.navigationController.navigationBar.bounds);
    self.tableView.frame = tableViewFrame;
